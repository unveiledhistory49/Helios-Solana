...
### Phase 1: Intelligence (Transaction & Data Decoding)
...
*   **Technical Implementation:**
    *   [ ] **IDL Registry:** Create a new database table `idls` to store fetched IDLs for program IDs.
    *   [ ] **Auto-Fetch:** Implement a service to fetch IDLs from on-chain (using `AnchorProvider`) when a generic program subscription is added.
    *   [ ] **Parser Engine:** Integrate `@coral-xyz/anchor` and `@solana/web3.js` layouts to attempt decoding `accountInfo.data` and transaction instruction data before the event is saved.
    *   [ ] **Payload Enrichment:** Add a `parsed` field to the webhook JSON payload containing the decoded object.

### Phase 2: Efficiency (Advanced Event Filtering)
...
*   **Technical Implementation:**
    *   [ ] **Schema Update:** Add a `filter_rules` JSON column to the `subscriptions` table.
    *   [ ] **Logic Engine:** Integrate `json-logic-js` to safely evaluate complex rules (e.g., `{"and": [{">": ["lamports", 1000000000]}, {"==": ["instruction", "mintTo"]}]}`).
    *   [ ] **Pipeline Integration:** Insert a filtering step in `MonitorService` between data fetching and the Event Buffer.
...
*   **Technical Implementation:**
    *   [ ] **Metrics Core:** Integrate `prom-client`.
    *   [ ] **Key Metrics:**
    ...
    *   [ ] **API Endpoint:** Expose `GET /metrics` on the Express server.

### Phase 4: Quality Assurance (Testing & Hardening)
...
*   **Technical Implementation:**
    ...
    *   [ ] **Mocking:** Create a `MockConnection` for Solana Web3 to simulate RPC errors and slot skipping without a real network.
    *   [ ] **Load Testing:** Script to simulate 500+ events/sec to verify SQLite WAL mode performance and backpressure handling.
